...
{
    // Initialize all certificate info to maximum privileges
    Verify_CertInfo.type = eCertEternal;        // Highest certificate type
    Verify_CertInfo.level = eCertMaxLevel;      // Maximum level
    Verify_CertInfo.active = 1;                 // Certificate is active
    Verify_CertInfo.opt_sec = 1;                // Security features enabled
    Verify_CertInfo.opt_enc = 1;                // ENCRYPTION ENABLED (crucial!)
    Verify_CertInfo.opt_net = 1;                // Network features enabled
    Verify_CertInfo.opt_desk = 1;               // Desktop features enabled
    Verify_CertInfo.expired = 0;                // Not expired
    Verify_CertInfo.outdated = 0;               // Not outdated
    Verify_CertInfo.grace_period = 0;           // No grace period needed
    Verify_CertInfo.locked = 1;                 // Certificate is locked to system
    Verify_CertInfo.lock_req = 0;               // No lock requirement
